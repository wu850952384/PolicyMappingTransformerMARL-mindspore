...
from msadapter.pytorch.optim import Adam
...
class PlasticSAC(object):
    ...
    def update_critic(self, sample, soft=True, logger=None, **kwargs):
        ...
        grad_fn = ms.ops.value_and_grad(forward_fn, None, self.critic_optimizer.parameters, has_aux=True)
        (loss, _), grads = grad_fn()
        self.critic_optimizer(grads)
    ...
    def update_policies(self, sample, soft=True, logger=None, **kwargs):
        ...
        for a_i, log_pi, q in zip(range(self.nagents),all_log_pis, critic_rets):
            ...
            grad_fn_a = ms.ops.value_and_grad(forward_fn, None, curr_agent.policy_optimizer.parameters, has_aux=True)
            (loss, _), grads = grad_fn_a()
            curr_agent.policy_optimizer(grads)
